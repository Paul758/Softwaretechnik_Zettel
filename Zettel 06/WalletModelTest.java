...
public class WalletModelTest {
	...
	//*Anfang* Übungszettel 06 Aufgabe 2)

	@Test
	public void testEmptyFlatList(){
	...
	@Test
	public void testFlatListWithSomeItems(){
		model = new WalletModel();

		//Setup objects
		WalletItem itemDaten = new WalletItem(ItemType.category, "Daten");
		WalletItem itemUni = new WalletItem(ItemType.item, "Uni");
		WalletItem itemBank = new WalletItem(ItemType.item, "Bank");

		//Set ID of objects
		itemDaten.setSysGUID("1");
		itemUni.setSysGUID("2");
		itemBank.setSysGUID("3");

		List<WalletItem> itemList = Arrays.asList(itemDaten, itemUni, itemBank);
		model.setItemsFlatList(itemList);
		model.buildTreeFromFlatList();

		//Check if the category is first entry in itemsFlatlist
		Assert.assertSame(itemDaten, model.getItemsFlatList().get(0));
		//Check if "Daten" category is the parent of "Uni" and "Bank" objects
		Assert.assertSame(itemDaten, itemUni.getParent());
		Assert.assertSame(itemDaten, itemBank.getParent());
		//Finally check if the order is as expected ("Uni" first and "Bank" second")
		Assert.assertSame(itemUni, itemDaten.getChildren().get(0));
		Assert.assertSame(itemBank, itemDaten.getChildren().get(1));

	}

    //*Ende* Übungszettel 06 Aufgabe 2
